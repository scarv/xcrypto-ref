...
% Co-processor name
\newcommandx{\COP}{\ISE Co-Processor }
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{image/cop-mem-if-1.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{image/top-level-fsm.pdf}
...
\subsection{SHA3 Block ({\tt scarv\_cop\_sha3})}

This block is responsible for implementing all of the SHA3 accelerator
instructions.
It's interface is described in table \ref{tab:sigs-sha3-block}.

\SIGNALS{
\SIGNALI{1}{sha3\_ivalid}{
    Indicates that the input instruction data is valid and that the 
    block should execute the specified instruction.
}
\SIGNALO{1}{sha3\_idone}{
    Indicates that the block has finished executing an instruction
    and is ready for another on the next cycle.
}
\SIGNALI{32}{sha3\_rs1}{RISC-V GPR source register 1}
\SIGNALI{32}{sha3\_rs2}{RISC-V GPR source register 2}
\SIGNALI{4}{id\_class}{
    Identifies which class of instruction the COP is executing.
}
\SIGNALI{3}{id\_subclass}{
...
\SIGNALI{32}{id\_imm}{
    Contains the shift amount for each of the SHA3 instructions.
}
\SIGNALO{4}{sha3\_cpr\_rd\_ben}{
    4-bit byte write enable signal for write-back data.
}
\SIGNALO{32}{sha3\_cpr\_wdata}{
    Data to be written to the $\GPR$ register file
}
\SIGNALO{32}{sha3\_cpr\_wen}{
    AES result $\CPR$ write enable.
}
}{Interface signals for the sha3 block. All signals are
synchronous to \SIGREF{g\_clk}.}
{tab:sigs-sha3-block}

The instructions handled by the sha3block are:
\begin{itemize}
\item {\tt xc.sha3.xy}
\item {\tt xc.sha3.x1}
\item {\tt xc.sha3.x2}
...
All of these instructions take 1 cycle to execute.

\subsection{Permutation Block ({\tt scarv\_cop\_permute})}

This block is responsible for implementing all of the bit permutation
instructions.
It's interface is described in table \ref{tab:sigs-permute-block}.

\SIGNALS{
\SIGNALI{1}{perm\_ivalid}{
    Indicates that the input instruction data is valid and that the 
    block should execute the specified instruction.
}
\SIGNALO{1}{perm\_idone}{
    Indicates that the block has finished executing an instruction
    and is ready for another on the next cycle.
}
\SIGNALI{32}{perm\_rs1}{RISC-V GPR source register 1}
\SIGNALI{32}{perm\_rs2}{
    RISC-V GPR source register 3 / destination register contents
}
\SIGNALI{3}{id\_subclass}{
    Identifies which instruction is being executed by the packed ALU block.
}
...
\SIGNALO{4}{perm\_cpr\_rd\_ben}{
    4-bit byte write enable signal for write-back data.
}
\SIGNALO{32}{perm\_cpr\_wdata}{
    Data to be written to the $\GPR$ register file
}
}{Interface signals for the permute block. All signals are
synchronous to \SIGREF{g\_clk}.}
{tab:sigs-permute-block}
...
\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{image/fsm-load-store.pdf}
...
\import{./tex/}{integration.tex}

\section{Verification}
\label{sec:verification}
\import{./tex/}{verify.tex}
